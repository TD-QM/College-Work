...
--    that has a Plocation of Houston). 
--    Note that the first condition is not equivalent to the employee's 
--    department having some Dlocation that is not in Houston the department 
--    must not have any Dlocation that is in Houston in order to be included 
--    in the result.
SELECT *
...
AND Employee.DNo IN (SELECT Department.DNum
					 FROM Department, Dept_Locations
					 WHERE Department.DNum = Dept_Locations.DNum 
					 AND 'Houston' NOT IN Dept_Locations.DLocation);
